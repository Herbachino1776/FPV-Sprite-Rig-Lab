...
export function LayerPanel({ layers, selectedLayerId, onSelect, onPatch, onRemove, onReorder }: LayerPanelProps) {
  ...
  return (
    <details className="panel controlSection layerPanel" open>
      <summary className="panelSummary"><span>Layers</span><small>{layers.length || 'No'} active layer{layers.length === 1 ? '' : 's'}</small></summary>
      <div className="panelBody">
        ...
        {selected && (
          <details className="nestedSection" open>
            <summary className="nestedSummary">Base Layer Setup</summary>
            <div className="controls">
              <p className="hint compact">Persistent base values shared by idle, slash, stab, spell, and future animations.</p>
              <input aria-label="Layer name" value={selected.name} onChange={(e) => onPatch(selected.id, { name: e.target.value })} />
              <label className="checkControl"><input type="checkbox" checked={selected.visible} onChange={(e) => onPatch(selected.id, { visible: e.target.checked })} /> Visible</label>
